...
case class TLV(tag: Short = ZERO.asInstanceOf[Short], value: Any = ZERO) {
  def pack(): List[Int] = {
    tag match {
      case ScInterfaceVersion => short2Binary(tag) ++ short2Binary(1) ++ sshort2Binary(INTERFACE_VERSION)
      case UserMessageReference => short2Binary(tag) ++ short2Binary(2) ++ short2Binary(value.asInstanceOf[Short])
  ...
  def unpack(data: List[Int]): TLV = {
    ...
    binary2Short(tag) match {
      ...
      case UserMessageReference => TLV(binary2Short(tag), binary2Short(value))
    }
  }
}
