...
class TestPDUData extends FlatSpec with ShouldMatchers {
  ...
  "int2Binary" should "give an List of Bytes" in {
    val expected: List[Byte] = List(0, 0, 2, 4)
  ...
  "short2Binary" should "give an List of Bytes" in {
    val expected: List[Byte] = List(4, 34)
  ...
  "binary2Int" should "give an Integer" in {
    val expected = 1058
    val result = binary2Int(List(0, 0, 4, 34))
    result should equal (expected)
  }

  "binary2Short" should " give a Short" in {
    val expected: Short = 516
    val result = binary2Short(List(2, 4))
    result should equal (expected)
  }

  "binary2String" should "give a String" in {
    val result = binary2String(List(97,98,99))
    val expected = "abc"
  ...
  it should "give an empty String" in {
    val result = binary2String(List(0))
    val expected = ""
    result.length should equal (0)
    //expected.length should equal (0)
  }

  "sshort2bytes" should "give an List of Bytes" in {
    val expected = List(2)
    val result = sshort2Binary(0x2)
    result should equal (expected)
  }

  "cstring2Binary" should "gve an List of Bytes, padded with 0" in {
    val expected: List[Byte] = List(111, 108, 117, 0)
  ...
  it should " also truncate extra characters if length is greater than 'max' arguement" in {
    val expected2: List[Byte] = List(111, 108, 0)
  ...
  "string2Binary" should "give an List of Bytes" in {
    val expected: List[Byte] = List(111, 108, 117)
    val result = string2Binary("olu", 5)
    result should equal (expected)
  }

  it should " also truncate extra extra characters if length is greater than 'max' arguement" in {
    val expected2: List[Byte] = List(111, 108)
    val result2 = string2Binary("olu", 2)

    result2 should equal (expected2)
  }

  "date2AbsoluteDate" should "give an List of Bytes" in {
    val cal = new java.util.GregorianCalendar();
    cal.set(2011, 0, 1, 15, 30, 3)
    val date = cal.getTime
    val expected = List(49, 49, 48, 49, 48, 49, 49, 53, 51, 48, 48, 51, 48, 48, 49, 43) //
    // "1101011530030001+"
    val result = date2AbsoluteDate(date)
    result should equal (expected)
  }

  "date2RelativeDate" should "give an List of Bytes" in {
    val expected = List(48, 48, 48, 48, 48, 48, 48, 48, 49, 53, 48, 48, 48, 48, 48, 82) //
    // "15mins = 49, 53"
